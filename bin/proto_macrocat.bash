...
if [ $help == 1 ]; then
  ...
  echo "-i --input       sets input file, must be two collumn ra, dec"
...
dbdir1=`echo $LSD_DB | sed 's/:/ /g' | awk '{print $1 }'`
if [ -e $dbdir1/$output ] ;then
  echo "$dbdir1/$output exists. Deleting."
  lsd-admin drop table $output
fi
...
if [ $nondecimal == 1 ]; then
  for num in `seq $nlines`; do
    if [ $input != 0 ]; then
      ...
      ra=`echo $blah | cut -f 1 -d ' '`
      dec=`echo $blah | cut -f 2 -d ' '`    
    fi
    if [ $nondecimal == 1 ]; then
    ...
    echo $ra $dec >> $output.temp
  done
else
	cat $input | awk '{ if ( $1 < 360 && $1 > 0 && $2 > -90 && $2 < 90 ) print $1, $2 }' > $output.temp
fi

#actually call lsd
lsd-admin create table --comp blosc --comp-level 5 --primary-key obj_id --spatial-keys ra,dec $output obj_id:u8 ra:f8 dec:f8
lsd-import text $output $output.temp
if [ $fields == 0 ]; then
	echo "lsd-query $outputoption 'select $output.ra as ra, $output.dec as dec, $db.* from $output, $db(matchedto=$output,nmax=1,dmax=$radius,outer)'" > $output.lsd
else
	echo "lsd-query $outputoption 'select $output.ra as ra, $output.dec as dec, $fields from $output, $db(matchedto=$output,nmax=1,dmax=$radius,outer)'" > $output.lsd
fi
rm -f $output.temp
